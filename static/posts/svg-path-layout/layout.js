...
function getT4Len(q, len, fullLen){
      var m = [q[3].x - q[0].x + 3 * (q[1].x - q[2].x),
               q[0].x - 2 * q[1].x + q[2].x,
               q[1].x - q[0].x];
      var n = [q[3].y - q[0].y + 3 * (q[1].y - q[2].y),
               q[0].y - 2 * q[1].y + q[2].y,
               q[1].y - q[0].y];
...
registerLayout('svg-path', class {
  ...
  async layout(children, edges, constraints, props) {
    ...
    const height = props.get('height').value;
    let viewbox = props.get('--viewbox');

    if (viewbox.length && viewbox[0].trim() != 'none') {
        viewbox = viewbox.toString().split(' ');
        viewbox = {
            min: {
                x: Number(viewbox[1]),
                y: Number(viewbox[2])
            ...
            max: {
                x: Number(viewbox[1]) + Number(viewbox[3]),
                y: Number(viewbox[2]) + Number(viewbox[4])
            }
        };
    }

    const layoutMode = props.get('--layout-mode').value;
    const aligns = {
        justify: props.get('--justify-items').value,
        align: props.get('--align-items').value
    };

    let bezierCurves = [];
    // get points from SVG path
    let points = pathDataToBezier(path);
    bezierCurves.push({ points: [ points[0] ] });
    for (let j = 1; j < points.length; j += 3) {
        bezierCurves.push({ points: [ points[j], points[j + 1], points[j + 2] ] })
    }
    ...
    const childFragmentsLength = childFragments.length;
    const lastPosition = pathLoop === 'loop' ? 0 : 1;

    for (var i = 0; i < childFragmentsLength; i++) {
        let childFragment = childFragments[i];

        const justifyAlign = children[i].styleMap.get('--justify-self').value;
        const alignAlign = children[i].styleMap.get('--align-self').value;

        const childAligns = {
            justify: justifyAlign === 'auto' ? aligns.justify : justifyAlign,
            align: alignAlign === 'auto' ? aligns.align : alignAlign
        };

        // where is this child on the bezier curve
        let lengthOnBeziers = Math.min(beziersLength, beziersLength / (childFragmentsLength - lastPosition) * i);

        let tmpBeziersLength = 0;
        for (var j = 0; j < beziers.length; j++) {
            const bezierLength = beziers[j].length();
            tmpBeziersLength += bezierLength;
            if (lengthOnBeziers <= tmpBeziersLength) {
                let t = getT4Len(beziers[j].points, bezierLength - (tmpBeziersLength - lengthOnBeziers), bezierLength);
                if (t === 0) {
                    t = 0.001;
                }
                let pt = beziers[j].get(t);
                // first translate bbox inside element
                pt.x += (bbox.width/2 + -(bbox.c.x));
                pt.y += (bbox.height/2 + -(bbox.c.y));
                // then scale
                pt.x *= scale;
                pt.y *= scale;
                ...
                // if fixed height, scale Y (no longer preserved aspect ratio)
                if (height != 'auto') {
                   pt.y *= height / autoBlockSize;
                }

                const currentSize = {
                    inline: childFragment.inlineSize,
                    block: childFragment.blockSize
                };

                if (layoutMode === 'contain') {
                    // all elements fit inside parent
                    pt.x -= currentSize.inline / (inlineSize / pt.x);
                    pt.y -= currentSize.block / (autoBlockSize / pt.oldY);
                } else if (layoutMode === 'absolute') {
                    // all elements are centered based on their properties
                    if (childAligns.justify === 'center') {
                        pt.x -= childFragment.inlineSize / 2;
                    } else if (childAligns.justify === 'anchor-end') {
                        pt.x -= childFragment.inlineSize;
                    }
                    if (childAligns.align === 'center') {
                        pt.y -= childFragment.blockSize / 2;
                    } else if (childAligns.align === 'anchor-end') {
                        pt.y -= childFragment.blockSize;
                    }
                }

                childFragment.inlineOffset = pt.x;
                childFragment.blockOffset = pt.y;
